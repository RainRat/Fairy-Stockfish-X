...
* Extended gating FEN masks (`...|<white>/<black>`) are parsed in `Position::set`; for non-commit gating, emit masks in `fen()` to preserve castling/gating round-trips consistently (including 8x8).
* `checking` and `allowChecks` are not interchangeable. When touching either, validate with at least one no-check variant and `racingkings` perft/search sanity to avoid silent legality/state drift.
...
* Fairy-Stockfish-X intentionally splits broad royal-danger from actual evasion-required check state. Preserve that split: search, movegen, mate/stalemate adjudication, and perpetual-check repetition logic use the evasion-only state, while pseudo-royal / anti-royal / legacy check-counting bookkeeping may still use the broader state.
...
* Spell context RAII must be nest-safe: if a scoped spell override is applied inside another spell-aware call (e.g., `legal()` -> `gives_check()`), guards must save/restore prior context, not just clear on scope exit.
* For variant-definition parsing, malformed input should be a hard error unless the syntax explicitly documents a sentinel such as `-`, `none`, or `0` for a valid clear/default state. Avoid silent fallback to inherited or default values on invalid text.
* For ambiguous rule combinations, prefer documented variant behavior first, then the human-intuitive reading, then the natural code path without feature-specific special cases. In rifle+atomic, the capture explosion is centered on the captured piece and the rifle piece survives unless it is actually in that blast area.
* If you add alternate repetition keys (`boardKey`-style), keep them synchronized in **all** state transitions, including `do_null_move()`, and reset matching repetition counters together.
* For reserve-aware repetition keys, mirror key composition exactly (hand + prison buckets as separate XOR terms). Avoid “merged count” shortcuts that don't algebraically cancel for PRISON layouts.
...
## 11) Research links (rules & precedent)
