...
## What it provides
- Move generation and legal move validation for all supported variants.
- FEN parsing and validation, SAN conversion, game result detection, piece reserve/pocket tracking, and variant metadata.
- Loads variant definitions directly from `variants.ini` at runtime.
- Pure native build with GCC, Clang, or MinGW (no WebAssembly or Emscripten required).
...
Notes:
- The Makefile disables NNUE embedding by default with `-DNNUE_EMBEDDING_OFF`. If you want NNUE embedded in the library, remove that flag and make sure the `.nnue` file is downloaded.
...
```
Make sure `libffish.so` is in your runtime library path (using `rpath` as shown above or by setting `LD_LIBRARY_PATH`).

### Windows (MinGW)
```
# after building ffish.dll and libffish.dll.a in src/
g++ example_dll_usage.cpp -Lpath/to/src -lffish -std=c++17 -O2 -o example.exe
```
Make sure `ffish.dll` is in the same folder as the executable or on `PATH`. Link against the import library `libffish.dll.a` generated by `Makefile_dll`.
...
- Windows (MinGW): `g++ demo.cpp -Lpath/to/src -lffish -std=c++17 -O2 -o demo.exe` (ensure `ffish.dll` is next to the exe or on `PATH`)
