...
fsx_harness_init() {
  ...
  case "${FSX_HARNESS_ENGINE_BASENAME}" in
    ...
    stockfish-compound*|stockfish-arimaa*)
      ...
      FSX_HARNESS_BUILD_ARGS=(ARCH=x86-64 largeboards=yes all=yes compoundturns=yes nnue=yes EXE="${FSX_HARNESS_ENGINE_BASENAME}")
      FSX_HARNESS_BUILD_EXE="${FSX_HARNESS_ENGINE_BASENAME}"
      ;;
    stockfish-allvars*)
      FSX_HARNESS_KNOWN_ENGINE_CONFIG=true
      FSX_HARNESS_CXX_DEFS+=(-DLARGEBOARDS -DPRECOMPUTED_MAGICS -DALLVARS -DNNUE_EMBEDDING_OFF -DENABLE_COMPOUND_TURNS)
      FSX_HARNESS_BUILD_ARGS=(ARCH=x86-64 largeboards=yes all=yes compoundturns=yes nnue=yes EXE=stockfish-allvars)
      FSX_HARNESS_BUILD_EXE=stockfish-allvars
      ;;
    stockfish-large*)
      FSX_HARNESS_KNOWN_ENGINE_CONFIG=true
      FSX_HARNESS_CXX_DEFS+=(-DLARGEBOARDS -DPRECOMPUTED_MAGICS -DALLVARS -DNNUE_EMBEDDING_OFF -DENABLE_COMPOUND_TURNS)
      FSX_HARNESS_BUILD_ARGS=(ARCH=x86-64 largeboards=yes all=yes compoundturns=yes EXE=stockfish-large)
    ...
    stockfish-vlb*)
      ...
      FSX_HARNESS_BUILD_ARGS=(ARCH=x86-64 largeboards=yes verylargeboards=yes all=yes compoundturns=yes nnue=yes EXE=stockfish-vlb)
