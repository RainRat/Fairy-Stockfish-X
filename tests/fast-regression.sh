...
run_step "hex boards" timeout 60s bash tests/test_hex_boards.sh "${ENGINE}"
...
run_step "anti-king" timeout 60s bash tests/anti-king.sh "${ENGINE}"
run_step "passive blast" timeout 60s bash tests/passive-blast.sh "${ENGINE}"
run_step "seega" timeout 60s bash tests/seega.sh "${ENGINE}" "${ROOT_DIR}/src/variants.ini"
...
run_step "swapping" timeout 60s bash tests/swapping.sh "${ENGINE}"
run_step "owner return hand" timeout 60s bash tests/owner-return-hand.sh "${ENGINE}"
