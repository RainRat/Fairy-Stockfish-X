...
do
    ref=$("$1" bench "$var" "${bench_args_arr[@]}" 2>&1 | grep "Nodes searched  : " | awk '{print $4}')
    signature=$("$2" bench "$var" "${bench_args_arr[@]}" 2>&1 | grep "Nodes searched  : " | awk '{print $4}')
    if [ -z "$ref" ]; then
